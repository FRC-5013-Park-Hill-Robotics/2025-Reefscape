...
public class goToPose extends Command {
  ...
  public void execute() {
    ...
    double OutputX = Math.min(Math.max(ControllerX.calculate(ErrorX), -DriveConstants.MaxSpeed), DriveConstants.MaxSpeed);
    double OutputY = Math.min(Math.max(ControllerY.calculate(ErrorY), -DriveConstants.MaxSpeed), DriveConstants.MaxSpeed);
    double OutputH = Math.min(Math.max(ControllerH.calculate(ErrorH), -DriveConstants.MaxAngularRate), DriveConstants.MaxAngularRate);
